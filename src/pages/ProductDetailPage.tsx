...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    if (found) {
      // Normalize colors if they are strings
      const normalizedProduct = {
        ...found,
        colors: found.colors.map((c: any) => typeof c === 'string' ? { name: c, code: '#CCCCCC' } : c),
        // Normalize images if strings
        images: found.images.map((img: any) => typeof img === 'string' ? { url: img } : img)
      };
      setProduct(normalizedProduct);
      if (normalizedProduct.sizes.length > 0) setSelectedSize(normalizedProduct.sizes[0]);
      if (normalizedProduct.colors.length > 0) setSelectedColor(normalizedProduct.colors[0].name);
    } else {
            ...
            <div className="hidden lg:grid grid-cols-5 gap-4">
              {filteredImages.map((img: any, idx: number) => (
                <button
                  ...
                  className={`aspect-square rounded-md overflow-hidden border-2 transition-all ${
                    activeImageIndex === idx && !showVideo ? 'border-vandora-emerald opacity-100' : 'border-transparent opacity-70 hover:opacity-100'
                  }`}
                >
              ...
              {hasVideo && (
                <button
                  ...
                  className={`aspect-square rounded-md overflow-hidden border-2 flex items-center justify-center bg-gray-100 transition-all ${
                    showVideo ? 'border-vandora-emerald' : 'border-transparent hover:bg-gray-200'
                  }`}
                >
                ...
                <div className="flex flex-wrap gap-3">
                  {product.sizes.map((size: string) => (
                    <button
                      ...
                      className={`min-w-[3rem] h-12 px-3 flex items-center justify-center border rounded-md text-sm font-medium transition-all ${
                        selectedSize === size
